...
export default function Transfer() {
	...
	const postTransfer = async () => {
		try {
			const { transferData } = await axios
				.post(`/api/projects/1/transfer`, {
					sender: user.info.id,
					recipient: "ramirc5",
					amount: 5,
				})
	...
	const transferComponentHandler = () => {
		switch (index) {
			case 0:
				return (
					<UserAmount data={transferData} index={index} setIndex={setIndex} />
				);
			case 1:
				return <Review data={transferData} index={index} setIndex={setIndex} />;
			case 2:
				return (
					<Complete data={transferData} index={index} setIndex={setIndex} />
				);
		}
	};
